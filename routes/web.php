...
// =====================
// PUBLIC / USER
// reg.complete: guest lolos, user yg belum selesai daftar diarahkan ke OTP/set-password
// =====================

Route::middleware(['reg.complete'])->group(function () {
    ...
    Route::get('/specialist/{pegawai_id}', [SpecialistController::class, 'show'])
        ->name('specialist.show')->whereNumber('pegawai_id');

});

// Route redirect & statis (tidak butuh reg.complete)
...
// Verifikasi email via OTP — tidak dibungkus reg.complete
...
// SET PASSWORD — tidak dibungkus reg.complete (ini tujuan redirect-nya)
Route::middleware(['auth'])->group(function () {
    Route::get ('/register/set-password', [AuthController::class, 'showSetPassword'])->name('register.set-password');
    Route::post('/register/set-password', [AuthController::class, 'setPassword'])->name('register.set-password');
});
...
Route::get('/forgotpw', function () { return view('login.forgotpw'); });
Route::get('/verif',    function () { return view('login.verif'); });
Route::get('/newpw',    function () { return view('login.newpw'); });
...
Route::middleware(['auth', 'reg.complete', 'role:owner'])->group(function () {
...
Route::middleware(['auth', 'reg.complete', 'role:admin'])
    ->prefix('admin')
    ->name('admin.')
    ->group(function () {
        ...
        Route::get('/pegawai', [PegawaiAdminController::class, 'index'])->name('pegawai');

        Route::get('/staff', function () {
            return redirect()->route('admin.pegawai');
        })->name('staff');

        Route::post('/pegawai/store', [PegawaiAdminController::class, 'store'])->name('pegawai.store');
        Route::put('/pegawai/{pegawai_id}', [PegawaiAdminController::class, 'update'])->name('pegawai.update');
        Route::delete('/pegawai/{pegawai_id}', [PegawaiAdminController::class, 'destroy'])->name('pegawai.destroy');

        Route::get('/pelanggan', [PelangganAdminController::class, 'index'])->name('pelanggan');
        Route::post('/pelanggan', [PelangganAdminController::class, 'store'])->name('pelanggan.store');
        Route::put('/pelanggan/{pelanggan_id}', [PelangganAdminController::class, 'update'])->name('pelanggan.update');
        Route::delete('/pelanggan/{pelanggan_id}', [PelangganAdminController::class, 'destroy'])->name('pelanggan.destroy');

        Route::get('/pengaturan', [PengaturanAdminController::class, 'index'])->name('pengaturan');
        Route::put('/pengaturan/profile', [PengaturanAdminController::class, 'updateProfile'])->name('pengaturan.profile.update');
        Route::put('/pengaturan/password', [PengaturanAdminController::class, 'updatePassword'])->name('pengaturan.password.update');

        Route::get('/input-promo', [InputPromoAdminController::class, 'index'])->name('inputpromo');
        Route::post('/input-promo/aktif', [InputPromoAdminController::class, 'activate'])->name('inputpromo.activate');
        Route::delete('/input-promo/aktif', [InputPromoAdminController::class, 'deactivate'])->name('inputpromo.deactivate');

        Route::get('/ulasan-saran', [UlasanAdminController::class, 'index'])->name('ulasan-saran');
        Route::put('/ulasan-saran/{ulasan_id}/status', [UlasanAdminController::class, 'updateStatus'])->name('ulasan-saran.status');
        Route::get('/ulasanadmin', function () {
            return redirect()->route('admin.ulasan-saran');
        })->name('ulasanadmin');

        Route::get('/layanan', [LayananAdminController::class, 'index'])->name('layanan');
        Route::post('/layanan/store', [LayananAdminController::class, 'store'])->name('layanan.store');
        Route::post('/layanan/jenis', [LayananAdminController::class, 'storeJenis'])->name('layanan.jenis.store');
        Route::put('/layanan/jenis/{id}', [LayananAdminController::class, 'updateJenis'])->name('layanan.jenis.update');
        Route::post('/layanan/paket', [LayananAdminController::class, 'storePaket'])->name('layanan.paket.store');
        Route::put('/layanan/paket/{id}', [LayananAdminController::class, 'updatePaket'])->name('layanan.paket.update');
        Route::patch('/layanan/{id}/deactivate', [LayananAdminController::class, 'deactivate'])->name('layanan.deactivate');
        Route::patch('/layanan/{id}/activate', [LayananAdminController::class, 'activate'])->name('layanan.activate');
        Route::put('/layanan/{id}', [LayananAdminController::class, 'update'])->name('layanan.update');

        Route::get('/layanan/{layanan_id}/album', [AlbumAdminController::class, 'index'])->name('album');
        Route::post('/layanan/{layanan_id}/album', [AlbumAdminController::class, 'store'])->name('album.store');
        Route::delete('/album/foto/{foto_id}', [AlbumAdminController::class, 'destroyFoto'])->name('album.foto.destroy');
    });
...
Route::middleware(['auth', 'reg.complete', 'role:pegawai'])
    ->prefix('pegawai')
    ->name('pegawai.')
    ->group(function () {
...
Route::middleware(['auth', 'reg.complete', 'role:pelanggan,owner,pegawai,admin'])
    ->prefix('pelanggan')
    ->name('pelanggan.')
    ->group(function () {
        ...
        Route::get('/promo/data', [PromoController::class, 'index'])->name('pelanggan.promo.data');
        ...
        Route::get('/booking/paket/{paket_id}/{cabang_id}', [BookingController::class, 'createFromPaket'])
            ->name('booking.paket')->whereNumber(['paket_id', 'cabang_id']);
