...
<div class="w-full px-4 py-4 font-sans text-[#3B302D]">
    ...
    @if($belumDibaca->isNotEmpty())
        ...
        @foreach($belumDibaca as $notif)
            ...
            <form action="{{ route('pegawai.notifikasi.dibaca', $notif->notifikasi_id) }}"
                  method="POST"
                  class="{{ !$loop->first ? 'mt-6' : '' }}">

                @csrf
                @method('PUT')

                <button type="submit"
                    class="w-full text-left border border-[#F1C9CE] rounded-[24px] px-8 py-5 flex justify-between items-start hover:shadow-md transition bg-white">
