        ...
        <form method="POST" action="{{ url('/register') }}" id="registerForm" novalidate>
            ...
            <div id="step1" class="step-panel active-panel relative">
                ...
                <div class="mb-4">
                    ...
                    <p class="error-msg text-red-500 text-xs mt-1 hidden" id="hpError">
                        <i data-feather="alert-circle" class="w-3 h-3 inline"></i>
                        Nomor WhatsApp tidak boleh kosong
                    </p>
                    <p class="text-[#3d352f]/50 text-xs mt-1">
                        Digunakan untuk verifikasi dan konfirmasi booking
                    </p>
                </div>
                ...
                <div class="flex items-center gap-3 my-4">
                    ...
                    <span class="text-xs text-[#3d352f]/50">atau daftar dengan</span>
                    <div class="flex-1 h-px bg-[#3d352f]/20"></div>
                </div>
...
<script>
    ...
    function validateStep1() {
        ...
        // No. HP — wajib, minimal 9 digit
        if (!noHp || noHp.length < 9) {
            showError('hpError', 'no_hp');
            valid = false;
        } else {
    ...
    // ========================
    // JUMP KE STEP YANG BENAR SAAT ADA ERROR DARI SERVER
    // ========================
    @if ($errors->has('password') || $errors->has('password_confirmation') || $errors->has('agree'))
        // Error di step 2 → langsung loncat ke step 2
        document.addEventListener('DOMContentLoaded', () => goToStep2());
    @elseif ($errors->has('nama') || $errors->has('email') || $errors->has('no_hp'))
        // Error di step 1 → tetap di step 1 (default), tidak perlu action
        // tapi trigger shake pada field yang error agar user langsung tahu
